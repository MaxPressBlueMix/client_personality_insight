...
var dw_Tooltip = {
		...
		hasSupport : function() {
			var doc = document;
			if (doc.createElement && doc.getElementsByTagName
		...
		setProps : function(dflts) {
			...
			var prop, el;
			if (!_this.defaultProps)
				_this.defaultProps = {};
			for (var i = 0; el = list[i]; i++) {
				prop = el[0];
				_this[prop] = (typeof _this.defaultProps[prop] == el[1]) ? _this.defaultProps[prop]
						: el[2];
			}
			if (!dflts) {
		...
		getDefaultsList : function() {
			return [ [ 'offX', 'number', 12 ], [ 'offY', 'number', 12 ],
					[ 'showDelay', 'number', 100 ], [ 'hideDelay', 'number', 100 ],
		...
		getPropsList : function() {
			return [ [ 'klass', 'string', '' ], [ 'followMouse', 'boolean', true ],
					[ 'sticky', 'boolean', false ],
					[ 'hoverable', 'boolean', false ], [ 'duration', 'number', 0 ],
					[ 'jumpAbove', 'boolean', true ],
					[ 'jumpLeft', 'boolean', true ], [ 'Left', 'boolean', false ],
					[ 'Above', 'boolean', false ],
		...
		activate : function(e, tgt, msg, id) {
			...
			console.log("activate: ",tgt,e.clientX, _this.location, _this.size);
//			if (e.clientX<_this.location)
//				return;
//			if (e.clientX>_this.location+_this.size)
//				return;
			if (!_this.tip.innerHTML)
				return;
			_this.active = true;
			_this.on_activate();
		...
		getContent : function(e, tgt, msg, id) {
			var _this = dw_Tooltip, vars, obj, loc, width;
			if (id && !msg) {
				...
				if (typeof obj == 'string') {
					...
					if (obj['content']) {
						...
						_this.size=obj['width'];
						console.log("getContent: ",_this.location, _this.size);
						}
					} else if (obj['html_id']) {
			...
			if (typeof e == 'object') {
				if (e.type == 'mouseover' || e.type == 'mousemove'
					...
					if (e.type == 'click') {
						...
						if ((_this.evX > (pos.x + tgt.offsetWidth))
								|| (_this.evX < pos.x)
								|| (_this.evY > (pos.y + tgt.offsetHeight))
								|| (_this.evY < pos.y)) {
							_this.evX = pos.x;
							_this.evY = pos.y;
							_this.foc = true;
		...
		deactivate : function(e) {
			...
			e = e ? e : window.event;
			if (e.type && e.type == 'mouseout'
					&& !dw_Util.mouseleave(e, _this.actuator)) {
				return;
			}
			_this.clearTimer('timer');
			_this.clearTimer('hoverTimer');
		...
		handleOptions : function(e) {
			...
			} else if (_this.followMouse
					&& !(e.type == 'focus' || e.type == 'focusin')) {
				dw_Event.add(document, 'mousemove', _this.positionRelEvent, true);
			} else if (!(_this.forTouch || _this.activateOnClick)
					&& (_this.sticky || _this.duration)) {
		...
		coordinateOptions : function() {
			...
			if (_this.forTouch || _this.activateOnClick || _this.sticky
					|| _this.hoverable || _this.duration
					|| _this.positionFn != _this.positionRelEvent) {
			...
			if ((_this.sticky || _this.forTouch || _this.activateOnClick)
					&& (_this.wrapFn == _this.plainWrap)) {
		...
		handleRestore : function() {
			...
			if (_this.followMouse) {
				dw_Event
						.remove(document, 'mousemove', _this.positionRelEvent, true);
			} else if (!(_this.forTouch || _this.activateOnClick)
					&& (_this.sticky || _this.duration)) {
		...
		checkDocClick : function(e) {
			...
			if (tgt === _this.actuator || tgt === tip) {
				...
				if (tgt.tagName && tgt.tagName.toLowerCase() == "img")
					tgt = tgt.parentNode;
				if (tgt.tagName.toLowerCase() != "a"
						|| tgt.href.indexOf("dw_Tooltip.hide") != -1)
					return;
			}
		...
		getActuatorInfo : function(tgt) {
			...
			if (!id && _this.content_source == 'class_id'
		...
		initHandlers : function() {
			...
			if (!_this.forTouch) {
				if (_this.activateOnClick) {
					...
					dw_Event.add(document, 'mouseover', _this.checkForActuator,
							true);
				}
				dw_Event.add(document, "keydown", _this.checkEscKey, true);
				if (!_this.activateOnClick && _this.activateOnFocus) {
					if (window.addEventListener) {
						dw_Event.add(document, 'focus', _this.checkForActuator,
								true);
					} else if (window.attachEvent) {
